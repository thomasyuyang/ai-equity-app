...
.setup-line, .metric-line, .notes, .links {
    font-size: 1rem;
    line-height: 1.45;
    margin: 7px 0;
}
...
def quick_label(kind: str, value: float):
    value = float(value)
    if kind == "timing":
        if value >= 8:
            return "🟢", "Good", "#e8f8ee"
    ...
    if kind == "risk":
        if value <= 3:
            return "🟢", "Low", "#e8f8ee"
        if value <= 6:
            return "🟡", "Normal", "#fff8db"
        return "🔴", "High", "#fde7e7"
    if kind == "trend":
        if value >= 8:
            return "🟢", "Strong", "#e8f8ee"
        if value >= 6:
            return "🟡", "Up", "#fff8db"
        return "🟠", "Weak", "#fff0df"
...
def distance_quick_label(dist: float):
    dist = float(dist)
    if dist > 8:
        return "🔴 Extended"
    if dist > 3:
...
def volatility_interpretation(row, percentile: float):
    trend = float(row["Trend_Score"])
    timing = float(row["Timing_Score"])
    risk = float(row["Risk_Score"])
    if trend >= 7:
        trend_msg = "🟢 Trend remains strong."
    elif trend >= 5:
        trend_msg = "🟡 Trend is constructive but not strong."
    else:
        trend_msg = "🟠 Trend is weak; wait for confirmation."

    if percentile >= 80:
        vol_msg = "🔴 Volatility is elevated but not necessarily bearish."
    elif percentile >= 60:
        vol_msg = "🟠 Volatility is above normal."
    else:
        vol_msg = "🟢 Volatility is normal or low."

    if timing >= 7 and risk <= 6:
        action_msg = "🟡 Suitable for staged buying, not aggressive all-in buying."
    elif risk >= 7:
        action_msg = "🟠 Consider waiting for a calmer pullback before adding."
    else:
        action_msg = "🟢 Position sizing can remain normal."
    return trend_msg, vol_msg, action_msg
...
def card(row, key_prefix="main"):
    ...
    timing_icon, timing_word, timing_bg = quick_label("timing", row["Timing_Score"])
    risk_icon, risk_word, risk_bg = quick_label("risk", row["Risk_Score"])
    trend_icon, trend_word, trend_bg = quick_label("trend", row["Trend_Score"])
    rsi_label = rsi_quick_label(row["RSI"])
    dist_label = distance_quick_label(row["Distance_to_EMA20_%"])

    # Keep the main card short and easy to scan on a phone.
    st.markdown(
        f'''
        <div class="stock-card" style="background-color:{background};">
            <div class="stock-title"><span class="status-square">{row['Emoji']}</span><span>{ticker} — {visual_status_text}</span></div>
            <div class="setup-line"><b>Setup:</b> {category}</div>

            <div class="metric-line"><b>Price:</b> ${row['Price']} &nbsp; <b>Score:</b> {row['Score']}/9 &nbsp; <b>Grade:</b> {row['Grade']}</div>

            <div class="pill-row">
                <span class="pill" style="background:{timing_bg};">{timing_icon} Timing {row['Timing_Score']}/10</span>
                <span class="pill" style="background:{risk_bg};">{risk_icon} Risk {row['Risk_Score']}/10</span>
                <span class="pill" style="background:{trend_bg};">{trend_icon} Trend {row['Trend_Score']}/10</span>
            </div>

            <div class="metric-grid">
                <div><b>RSI</b><br>{row['RSI']}<br><span class="hint">{rsi_label}</span></div>
                <div><b>ATR%</b><br>{row['ATR_%']}%<br><span class="hint">Daily swing</span></div>
                <div><b>Vol20</b><br>{row['Vol20_%']}%<br><span class="hint">20D annual vol</span></div>
                <div><b>EMA20</b><br>{row['EMA20']}<br><span class="hint">20-day trend</span></div>
                <div><b>EMA50</b><br>{row['EMA50']}<br><span class="hint">50-day trend</span></div>
                <div><b>Dist EMA20</b><br>{row['Distance_to_EMA20_%']}%<br><span class="hint">{dist_label}</span></div>
            </div>

            <div class="notes"><b>Notes:</b> {notes}</div>
            <div class="links">
    ...
    with st.expander(f"📊 {ticker} Dashboards: Timing | Risk | Volatility", expanded=False):
        ...
        with tab1:
            ...
            c2.metric("Dist EMA20", f"{row['Distance_to_EMA20_%']}%", distance_quick_label(row["Distance_to_EMA20_%"]))

            st.markdown("#### Suggested Buy Levels")
            b1, b2, b3 = st.columns(3)
            b1.metric("Aggressive", f"${row['Aggressive_Buy']:.2f}")
            b2.metric("Normal", f"${row['Normal_Buy']:.2f}")
            b3.metric("Strong", f"${row['Strong_Buy']:.2f}")
            st.info("Best intraday windows to check: 10:30–11:30 ET and 2:00–3:30 ET.")
        ...
        with tab2:
            ...
            c3, c4 = st.columns(2)
            c3.metric("RSI", row["RSI"], rsi_quick_label(row["RSI"]))
            c4.metric("BB Width", f"{row['BB_Width_%']}%")
            vol_pct_text = "N/A" if row["Volatility_Percentile"] is None else f"{row['Volatility_Percentile']}%"
            st.metric("Volatility Percentile", vol_pct_text)
            st.caption("Risk score is overall setup risk. Volatility percentile only measures how unusual recent volatility is.")
        ...
        with tab3:
            ...
            if vol_df is None or vol_df.empty:
                ...
                status_text, vol_status = volatility_status_label(latest_vol, avg_30, percentile)
                ...
                c4.metric("Vol status", vol_status)

                st.markdown("### Interpretation")
                trend_msg, vol_msg, action_msg = volatility_interpretation(row, percentile)
                ...
                fig.update_layout(height=300, margin=dict(l=6, r=6, t=25, b=6), yaxis_title="Annualized vol %", xaxis_title="Time")
...
st.caption("Mobile-friendly quick-scan AI stock/ETF setup scanner")
