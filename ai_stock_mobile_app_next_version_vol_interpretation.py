...
''')
        ...
        with tab3:
            ...
            if vol_df is None or vol_df.empty:
                ...
                # Human-readable volatility interpretation
                if row["Trend_Score"] >= 7:
                    trend_msg = "🟢 Trend remains strong."
                elif row["Trend_Score"] >= 5:
                    trend_msg = "🟡 Trend remains constructive."
                else:
                    trend_msg = "🔴 Trend is weakening."

                if percentile >= 70:
                    vol_msg = "🔴 Volatility is elevated but not necessarily bearish."
                elif percentile >= 40:
                    vol_msg = "🟡 Volatility is normal to moderately elevated."
                else:
                    vol_msg = "🟢 Volatility is relatively low."

                if row["Timing_Score"] >= 7 and row["Risk_Score"] <= 6:
                    action_msg = "🟡 Suitable for staged buying,\n   not aggressive all-in buying."
                elif row["Risk_Score"] >= 7:
                    action_msg = "🟠 Consider waiting for a better pullback before adding."
                else:
...
""")
                ...
                b1.metric("Aggressive Buy", f"${row['Aggressive_Buy']:.2f}")
                b2.metric("Normal Buy", f"${row['Normal_Buy']:.2f}")
                b3.metric("Strong Buy", f"${row['Strong_Buy']:.2f}")
