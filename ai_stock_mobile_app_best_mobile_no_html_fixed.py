...
def ai_verdict(row):
    """Simple final decision text for the mobile quick-scan card."""
    timing = float(row["Timing_Score"])
    risk = float(row["Risk_Score"])
    trend = float(row["Trend_Score"])
    ...
    if timing >= 8 and risk <= 5 and trend >= 7 and dist <= 3:
        return "🟢 GOOD BUY CANDIDATE", "Trend strong; risk controlled; near EMA20."
    if timing >= 7 and risk <= 6 and trend >= 5:
        return "🟡 STAGED BUY / WATCH", "Good timing, but use partial/staged buying."
    if risk >= 8 or dist > 8:
        return "🔴 WAIT", "Risk or extension is high; avoid chasing."
    if trend < 5:
        return "🟠 NEUTRAL", "Trend is not strong yet; wait for confirmation."
...
def card(row, key_prefix="main"):
    ...
    timing_icon, timing_word, _ = quick_label("timing", row["Timing_Score"])
    risk_icon, risk_word, _ = quick_label("risk", row["Risk_Score"])
    trend_icon, trend_word, _ = quick_label("trend", row["Trend_Score"])
    ...
    # Native Streamlit card: no HTML, so it will not display raw HTML code.
    with st.container(border=True):
        st.markdown(f"### {row['Emoji']} {ticker}")
        st.caption(f"{category} · {visual_status_text}")

        top_left, top_right = st.columns([1.35, 1])
        with top_left:
            st.metric("Price", f"${row['Price']}", f"Score {row['Score']}/9 · {row['Grade']}")
        with top_right:
            st.metric("Timing", f"{row['Timing_Score']}/10", f"{timing_icon} {timing_word}")

        c1, c2 = st.columns(2)
        with c1:
            st.metric("Risk", f"{row['Risk_Score']}/10", f"{risk_icon} {risk_word}")
        with c2:
            st.metric("Trend", f"{row['Trend_Score']}/10", f"{trend_icon} {trend_word}")

        st.markdown("**Quick read**")
        st.markdown(
            f"{rsi_label} **RSI {row['RSI']}** · "
            f"🟡 **Daily swing ±${row['ATR']:.2f}** · "
            f"🟡 **Vol20 {row['Vol20_%']}%**"
        )
        st.markdown(
            f"EMA20 **{row['EMA20']}** · EMA50 **{row['EMA50']}** · "
            f"{dist_label} **Dist EMA20 {row['Distance_to_EMA20_%']}%**"
        )

        if "GOOD BUY" in verdict:
        ...
        with l1:
            st.link_button("Yahoo Chart", row["Yahoo Chart"], use_container_width=True)
        with l2:
            st.link_button("Yahoo Quote", row["Yahoo Quote"], use_container_width=True)
        with l3:
            st.link_button("Fidelity", row["Fidelity"], use_container_width=True)
    ...
    with st.expander(f"📊 {ticker} Dashboards: Timing | Risk | Volatility", expanded=False):
        ...
        with tab3:
            ...
            interval_label = st.radio(
                ...
                ["5m", "30m", "60m", "1d", "1wk"],
                horizontal=True,
                key=f"interval_{key_prefix}_{ticker}",
            )
