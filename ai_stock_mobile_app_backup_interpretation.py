        ...
        with tab3:
            ...
            if vol_df is None or vol_df.empty:
                ...
                if row["Trend_Score"] >= 7:
                    st.success("🟢 Trend remains strong.")
                elif row["Trend_Score"] >= 5:
                    st.info("🟡 Trend is neutral or weakening.")
                else:
                    st.warning("🔴 Trend is weak.")

                if percentile >= 80:
                    st.warning("🔴 Volatility is elevated but not necessarily bearish.")
                else:
                    st.info("🟡 Volatility is within normal range.")

                if row["Timing_Score"] >= 7:
                    st.info("🟡 Suitable for staged buying; not aggressive all-in buying.")
                else:
                    st.info("🟠 Consider waiting for a better entry before adding aggressively.")

                st.markdown("### Suggested Buy Levels")

                b1, b2 = st.columns(2)
                with b1:
                    st.metric("Aggressive", f"${row['Aggressive_Buy']:.2f}")
                with b2:
                    st.metric("Normal", f"${row['Normal_Buy']:.2f}")

                st.metric("Strong Buy", f"${row['Strong_Buy']:.2f}")
