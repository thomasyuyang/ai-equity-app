...
def card(row, key_prefix="main"):
    """Display one stock card using native Streamlit components only.
    This avoids the old bug where HTML code was shown on the page.
    """
    ...
    with st.container(border=True):
        st.subheader(f"{row['Emoji']} {ticker} — {visual_status_text}")
        st.caption(f"Setup: {category}")

        c1, c2, c3 = st.columns(3)
        c1.metric("Price", f"${row['Price']}")
        c2.metric("Score", f"{row['Score']}/9")
        c3.metric("Grade", row["Grade"])

        c4, c5, c6 = st.columns(3)
        c4.metric("Timing", f"{row['Timing_Score']}/10")
        c5.metric("Risk", f"{row['Risk_Score']}/10")
        c6.metric("Trend", f"{row['Trend_Score']}/10")

        st.markdown("**AI Interpretation**")
        st.write(f"{simple_phrase(row['Timing_Score'], 'timing')}: {row['Timing_Score']}/10")
        st.write(f"{simple_phrase(row['Trend_Score'], 'trend')}: {row['Trend_Score']}/10")
        st.write(f"{simple_phrase(row['Risk_Score'], 'risk')}: {row['Risk_Score']}/10")
        st.write(f"{rsi_explain(row['RSI'])} RSI: {row['RSI']}")
        st.write(f"🟡 Daily Swing: ±${float(row['ATR']):.2f} (ATR {row['ATR_%']}%)")
        st.write(f"{vol_explain(row['Vol20_%'])} Volatility: {row['Vol20_%']}%")
        st.write(f"{distance_explain(row['Distance_to_EMA20_%'])} Distance to EMA20: {row['Distance_to_EMA20_%']}%")

        verdict, verdict_reason = ai_verdict(row)
        if verdict.startswith("🟢"):
            st.success(f"AI Verdict: {verdict}\n\n{verdict_reason}")
        elif verdict.startswith("🟡"):
            st.warning(f"AI Verdict: {verdict}\n\n{verdict_reason}")
        elif verdict.startswith("🔴"):
        ...
        l1.link_button("Yahoo Chart", row["Yahoo Chart"], use_container_width=True)
        l2.link_button("Yahoo Quote", row["Yahoo Quote"], use_container_width=True)
    ...
    with st.expander(f"📊 {ticker} Dashboards: Timing | Risk | Volatility", expanded=False):
        ...
        with tab1:
            ...
            c2.metric("Distance to EMA20", f"{row['Distance_to_EMA20_%']}%", distance_explain(row["Distance_to_EMA20_%"]))
            st.markdown(f"""
**Suggested buy levels based on ATR**

- Aggressive buy: **${float(row['Aggressive_Buy']):.2f}**
- Normal buy: **${float(row['Normal_Buy']):.2f}**
- Strong buy: **${float(row['Strong_Buy']):.2f}**
...
""")
        ...
        with tab3:
            ...
            if vol_df is None or vol_df.empty:
                ...
                c4.metric("Volatility Status", risk_level)
                ...
                action_msg = "🟡 Suitable for staged buying,\nnot aggressive all-in buying." if float(row["Timing_Score"]) >= 7 else "🟠 Consider waiting for a better entry."
                ...
                b1.metric("Aggressive Buy", f"${float(row['Aggressive_Buy']):.2f}")
                b2.metric("Normal Buy", f"${float(row['Normal_Buy']):.2f}")
                b3.metric("Strong Buy", f"${float(row['Strong_Buy']):.2f}")
